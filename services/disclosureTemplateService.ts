...
const ADVANTAGE_QUESTION_KEYWORDS =
  /效果|性能|测试|可靠性|成本|收率|准确率|效率|活性|安全|临床/;
const ALTERNATIVE_STATEMENT_KEYWORDS =
  /替代|替换|可替换|可选|备选|变体|改用|另一实施例|另一种实现|优选方案/;
...
const extractAlternativeStatements = (answer: string): string[] =>
  answer
    ...
    .map((item) => item.trim())
    .filter(
      (item) => item.length > 0 && ALTERNATIVE_STATEMENT_KEYWORDS.test(item),
    );

const joinDisclosureText = (...items: Array<string | undefined>): string =>
  items
    .map((item) => item?.trim() || "")
    .filter(Boolean)
    .join("\n\n");

const mergeDisclosureItems = (
  ...
  dedupeList(
    items.flatMap((item) => {
      ...
      if (Array.isArray(item)) {
        return item.map((entry) => entry.trim()).filter(Boolean);
      }

      return splitAnswerItems(item);
    }),
  );

const buildImportSnapshotNotes = (
  importSnapshot?: DisclosureData["importSnapshot"],
): string[] => {
  if (!importSnapshot) {
    return [];
  }
  ...
  return [
    importSnapshot.sourceSummary.trim()
      ? `【上传资料摘要】:\n${importSnapshot.sourceSummary}`
      : "",
    importSnapshot.technicalProblem.trim()
      ? `【上传资料识别的技术问题】:\n${importSnapshot.technicalProblem}`
      : "",
    importSnapshot.existingSolutionIssues.trim()
      ? `【上传资料识别的现有技术不足】:\n${importSnapshot.existingSolutionIssues}`
      : "",
    importSnapshot.keyPoints.length > 0
      ? `【上传资料提炼的保护要点】:\n${importSnapshot.keyPoints.join("\n")}`
      : "",
    importSnapshot.evidenceMaterials.length > 0
      ? `【上传资料中的证据材料】:\n${importSnapshot.evidenceMaterials.join("\n")}`
      : "",
  ].filter(Boolean);
};
...
export const buildDisclosurePatentFields = (
  ...
  | "evidenceMaterials"
> => {
  const questions = generateDeepQuestionnaire(
    disclosureData.type,
    disclosureData.field,
  );
  const questionMap = new Map(
    questions.map((question) => [question.id, question]),
  );
  const importSnapshot = disclosureData.importSnapshot;
  const answerMap = disclosureData.answers.reduce<Record<string, string>>(
    (accumulator, item) => {
      if (item.answer.trim()) {
  ...
  const getPreferredAnswer = (questionId: string, fallback?: string): string =>
    getAnswer(questionId).trim() || fallback?.trim() || "";

  const disclosureNotes = [
    ...buildImportSnapshotNotes(importSnapshot),
    ...questions
      .map((question) => {
    ...
    ...disclosureData.answers
      ...
      .map(
        (item) => `【问题】: ${item.questionId}\n【回答】: \n${item.answer}`,
      ),
  ].join("\n\n");

  if (disclosureData.type === "utility") {
    const structureDescription = getPreferredAnswer(
      "q4",
      joinDisclosureText(
        importSnapshot?.embodiments.join("\n"),
        importSnapshot?.sourceSummary,
      ),
    );
    ...
    return {
      disclosureNotes,
      technicalProblem: joinDisclosureText(
        getPreferredAnswer("q1", importSnapshot?.technicalProblem),
        getAnswer("q3"),
      ),
      existingSolutionIssues: getPreferredAnswer(
        "q2",
        importSnapshot?.existingSolutionIssues,
      ),
      disclosureSummary: structureDescription,
      technicalHighlights: mergeDisclosureItems(
        importSnapshot?.technicalHighlights,
        importSnapshot?.keyPoints,
      ),
      embodiments: mergeDisclosureItems(
        structureDescription,
        importSnapshot?.embodiments,
      ),
      advantages: mergeDisclosureItems(
        getAnswer("q5"),
        importSnapshot?.advantages,
      ),
      alternativeSolutions: dedupeList([
        ...disclosureData.answers.flatMap((item) =>
          extractAlternativeStatements(item.answer),
        ),
        ...(importSnapshot?.alternativeSolutions || []),
      ]),
      evidenceMaterials: mergeDisclosureItems(
        importSnapshot?.evidenceMaterials,
      ),
    };
  }
  ...
  const evidenceMaterials = mergeDisclosureItems(
    ...evidenceQuestions.map((question) => getAnswer(question.id)),
    importSnapshot?.evidenceMaterials,
  );
  const embodiments = mergeDisclosureItems(
    getAnswer("q3"),
    ...fieldSpecificQuestions.map((question) => getAnswer(question.id)),
    importSnapshot?.embodiments,
  );
  ...
  return {
    disclosureNotes,
    technicalProblem: getPreferredAnswer(
      "q1",
      importSnapshot?.technicalProblem,
    ),
    existingSolutionIssues: getPreferredAnswer(
    ...
    disclosureSummary: getPreferredAnswer("q3", importSnapshot?.sourceSummary),
    technicalHighlights: mergeDisclosureItems(
      getAnswer("q4"),
      importSnapshot?.technicalHighlights,
      importSnapshot?.keyPoints,
...
const getInventionBaseQuestions = (): TemplateQuestion[] => [
  {
    ...
    helpText:
      "用一句话概括发明要达成的目标，即现有技术无法解决或解决不好的问题",
  ...
  },
  {
    ...
    helpText:
      "描述现有解决方案及其缺陷，可以从性能、效率、成本、稳定性等维度说明",
    exampleAnswer:
      "现有的深度学习分割网络在边缘区域存在精度下降的问题，且计算复杂度较高，无法满足实时性要求",
  ...
  },
  {
    ...
    exampleAnswer:
      "采用基于注意力机制的双分支网络结构，分别提取特征并进行融合；引入边界增强模块利用边缘信息提升分割精度；通过轻量化设计降低计算量",
  ...
  },
  {
    ...
    exampleAnswer:
      "1) 提出了边界感知的注意力模块；2) 设计了特征金字塔融合策略；3) 实现了计算量与精度的平衡",
    placeholder: "列出主要创新点",
  },
];
 ...
 * 获取技术领域特定问题
 */
const getFieldSpecificQuestions = (
    ...
    AI: [
      {
        ...
        exampleAnswer:
          "采用改进的Transformer架构，包含12层编码器和6层解码器；使用GELU激活函数；损失函数为交叉熵+KL散度；优化器为AdamW，学习率0.0001",
      ...
      },
      {
        ...
        exampleAnswer:
          "使用ImageNet-1K数据集（120万张训练图片）；预处理包括随机裁剪、翻转、归一化；采用Mixup数据增强",
      ...
      },
      {
        ...
        exampleAnswer:
          "使用ONNX格式部署到GPU服务器；单张图片推理时间15ms；吞吐量66 FPS；显存占用2GB",
    ...
    新能源: [
      {
        ...
        exampleAnswer:
          "通过三元锂电池实现电能存储，能量密度达到280Wh/kg；充电效率95%；循环寿命2000次以上",
      ...
      },
      {
        ...
        exampleAnswer:
          "正极材料为NCM811（镍钴锰比例8:1:1），负极为石墨；电解液为六氟磷酸锂；成本约500元/kWh",
      ...
      },
      {
        ...
        exampleAnswer:
          "采用陶瓷涂层隔膜防止短路；通过针刺、挤压、过充测试；符合GB 38031-2020标准",
    ...
    医疗器械: [
      {
        ...
        exampleAnswer:
          "用于心血管疾病诊断；通过超声探头扫描获取心脏图像；自动分析心脏功能参数；操作时间约5分钟",
      ...
      },
      {
        ...
        exampleAnswer:
          "接触材料为医用硅胶和316L不锈钢；通过ISO 10993生物相容性测试；采用环氧乙烷灭菌",
      ...
      },
      {
        ...
        exampleAnswer:
          "完成120例临床试验；诊断准确率95%；敏感性92%，特异性97%；p<0.01显著优于对照组",
    ...
    软件: [
      {
        ...
        exampleAnswer:
          "采用微服务架构；包含用户管理、订单处理、支付网关、消息队列等模块；通过gRPC通信",
        placeholder: "描述系统架构",
      },
      {
        id: "q_software_2",
        ...
        exampleAnswer:
          "使用改进的A*算法进行路径规划；时间复杂度O(n log n)；通过启发式函数优化减少搜索空间50%",
        placeholder: "描述核心算法",
      },
      {
        id: "q_software_3",
        ...
        exampleAnswer:
          "平均响应时间100ms；支持10万并发；可用性99.9%；采用主备冗余+自动故障转移",
    ...
    机械: [
      {
        ...
        exampleAnswer:
          "包含齿轮传动机构和凸轮机构；通过皮带传动实现动力传递；齿轮比1:3.5；凸轮轮廓为椭圆形",
      ...
      },
      {
        ...
        exampleAnswer:
          "主体材料为45号钢；屈服强度355MPa；经有限元分析，最大应力280MPa；安全系数1.27",
        placeholder: "描述材料和强度",
      },
      {
        id: "q_mech_3",
        ...
        exampleAnswer:
          "采用数控铣削加工；公差等级IT7；表面粗糙度Ra1.6；关键部位淬火处理提高硬度",
        placeholder: "描述加工工艺",
      },
    ],
    化工: [
      {
        ...
        exampleAnswer:
          "2A + B → C + D；反应温度180°C；压力2MPa；催化剂为铂碳；反应时间4小时；产率92%",
        placeholder: "描述化学反应",
      },
      {
        id: "q_chem_2",
        ...
        exampleAnswer:
          "产物为白色晶体；熔点125-127°C；易溶于乙醇，微溶于水；常温常压稳定；低毒性，LD50>2000mg/kg",
        placeholder: "描述理化性质",
      },
      {
        id: "q_chem_3",
        ...
        exampleAnswer:
          "通过回流冷凝回收溶剂，溶剂损失从15%降至3%；优化催化剂用量从5%降至2%；综合成本降低25%",
        placeholder: "描述工艺优化",
      },
    ],
    电子: [
      {
        ...
        exampleAnswer:
          "采用双运放差分放大电路；运放选用OPA2134；增益60dB；带宽20Hz-20kHz；THD<0.001%",
        placeholder: "描述电路设计",
      },
      {
        id: "q_elec_2",
        ...
        exampleAnswer:
          "工作电压5V±10%；静态电流50μA；峰值电流200mA；待机功耗0.25mW；转换效率90%",
        placeholder: "描述电气参数",
      },
      {
        id: "q_elec_3",
        ...
        exampleAnswer:
          "通过24位ADC采样；采样率192kHz；使用8阶Butterworth低通滤波；截止频率20kHz；动态范围120dB",
        placeholder: "描述信号处理",
      },
    ],
    通信: [
      {
        ...
        exampleAnswer:
          "采用LoRa物联网协议；使用扩频因子SF7；数据帧包含帧头（2字节）+负载（最大255字节）+CRC（2字节）",
        placeholder: "描述通信协议",
      },
      {
        id: "q_comm_2",
        ...
        exampleAnswer:
          "空旷环境传输距离5km；数据速率5kbps；误码率<10^-6；支持多径衰落和频偏补偿",
        placeholder: "描述传输性能",
      },
      {
        id: "q_comm_3",
        ...
        exampleAnswer:
          "采用星型网络拓扑；网关支持1024个终端节点；使用TDMA时分多址避免冲突；支持动态休眠降低功耗",
        placeholder: "描述网络拓扑",
      },
    ],
    生物: [
      {
        ...
        exampleAnswer:
          "使用大肠杆菌BL21菌株表达重组蛋白；质粒为pET-28a；保存于-80°C甘油菌种；蛋白分子量45kDa",
        placeholder: "描述生物材料",
      },
      {
        id: "q_bio_2",
        ...
        exampleAnswer:
          "使用PCR扩增目的基因；限制性内切酶EcoRI和XhoI双酶切；Western blot检测蛋白表达；ELISA测定活性",
        placeholder: "描述实验方法",
      },
      {
        id: "q_bio_3",
        ...
        exampleAnswer:
          "体外抗肿瘤活性测试，IC50为2.5μM；细胞凋亡率78%；动物实验肿瘤体积缩小65%（p<0.01）",
        placeholder: "描述生物活性",
      },
    ],
    材料: [
      {
        ...
        exampleAnswer:
          "采用溶胶凝胶法制备；前驱体溶液浓度0.5M；80°C干燥12h；600°C煅烧2h；冷却速率5°C/min",
        placeholder: "描述材料制备",
      },
      {
        id: "q_mat_2",
        ...
        exampleAnswer:
          "通过XRD确认为立方晶系；SEM观察颗粒尺寸50-100nm；BET比表面积280m²/g；孔径分布集中在2-5nm",
        placeholder: "描述微观结构",
      },
      {
        id: "q_mat_3",
        ...
        exampleAnswer:
          "抗拉强度580MPa，比普通材料提高40%；导电率1.2×10^6 S/m；热膨胀系数5×10^-6/K；循环稳定性1000次无明显衰减",
        placeholder: "描述性能测试",
      },
    ],
  };
...
const getEvidenceQuestions = (field: TechnicalField): TemplateQuestion[] => [
  {
    ...
    helpText:
      "必须包含具体数字、单位、测试条件。格式：指标名称 + 数值 + 单位 + 测试条件",
    exampleAnswer:
      field === "AI"
  ...
  },
  {
    ...
    exampleAnswer:
      "可靠性测试：MTBF达到50000小时；成本：相比现有方案降低30%（500元 → 350元）；用户满意度：4.8/5.0（n=200）",
...
const getUtilityModelQuestions = (): TemplateQuestion[] => [
  ...
  },
  {
    ...
    exampleAnswer:
      "现有折叠伞采用六根伞骨支撑，收纳后直径约30cm，且伞柄容易松动",
  ...
  },
  {
    ...
    exampleAnswer:
      "包括伞骨组件、连接件和收纳盒；伞骨采用四级伸缩结构；连接件采用卡扣式设计...",
